...
const App = () => {
  ...
  const urheilulajit = ["juoksu", "uinti", "jalkapallo", "koripallo", "tennis", "sulkapallo", "koripallo",
    "kuntosaliharjoittelu", "judo", "futsal", "salibandy", "nyrkkeily", "padel", "squash", ""]

  const taidelajit = ["maalaaminen", "kuvanveisto", "keramiikan teko", "ompelu", "neuolominen", "korujen tekeminen"]

  const musiikkilajit = ["soittaminen", "räppääminen", "kuorossa laulaminen", "bändissä soittaminen", "DJ soittaminen"]

  const tanssilajit = ["tanssi", "baletti", "zumba", "breakdance", "tango tanssi"]

  const pelaaminen = ["tietokonepelaaminen", "konsolipelaaminen", "lautapelailu", "korttipelaaminen", "Dungeons and Dragons"]

  const luontolajit = ["retkeily", "kalastus", "metsästys", "puutarhanhoito", "patikointi"]

  const ruokaharrastukset = ["kokkaaminen", "leipominen", "viininvalmistus", "oluenpano", "puutarhanhoito"]

  const matkailuharrastukset = ["vaellus", "maastopyöräily", "sukellus", "surffaus", "maantipyöräily", "laskettelu"]

  const yksin = ["Lukeminen", "Kirjoittaminen", "Piirtäminen", "maalaaminen", "Valokuvaus", "soittaminen", "juokseminen",
  ...
  function calculateHobby() {
    ...
    if (yleinenKunto) {
      currentPool.push(urheilulajit[Math.floor(Math.random() * urheilulajit.length)]);
      currentPool.push(tanssilajit[Math.floor(Math.random() * tanssilajit.length)]);
    }
    if (kilpailu) {
      currentPool.push(urheilulajit[Math.floor(Math.random() * urheilulajit.length)]);
    }
    if (luova) {
      currentPool.push(taidelajit[Math.floor(Math.random() * taidelajit.length)]);
      currentPool.push(ruokaharrastukset[Math.floor(Math.random() * ruokaharrastukset.length)]);
    }
    if (uudetTaidot) {
      currentPool.push(taidelajit[Math.floor(Math.random() * taidelajit.length)]);
      currentPool.push(tanssilajit[Math.floor(Math.random() * tanssilajit.length)]);
  ...
  const onSubmit = () => {
    ...
    if (harrastus.length < 1) {
      ...
      alert("Sinulle arvottu laji on: " + harrastus[Math.floor(Math.random() * harrastus.length)])
    }
  }
